...
export default function PregnantWomenList() {
  const { women, isLoading, deleteWoman } = usePregnantWomen();
  const { camps } = useCamps();
  const [searchTerm, setSearchTerm] = useState("");
  const [womanToDelete, setWomanToDelete] = useState<number | null>(null);
  ...
  const filteredWomen = women?.filter(woman =>
    ...
    (woman.contactNumber && woman.contactNumber.includes(searchTerm))
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-3xl font-bold">سجلات الأمهات</h2>
          <p className="text-muted-foreground">إدارة سجلات صحة النساء الحوامل.</p>
        </div>
        <Link href="/pregnant-women/add">
          <Button
            className="bg-accent hover:bg-accent/90 text-white shadow-lg shadow-accent/20"
          >
            إضافة سجل <Plus className={`${direction === 'rtl' ? 'mr-2' : 'ml-2'} w-4 h-4`} />
          </Button>
        </Link>
      </div>
          ...
          <table className="w-full text-sm text-right"> {/* Changed text-left to text-right for RTL */}
            <thead className="text-xs text-muted-foreground uppercase bg-secondary/50 border-b">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">شهر الحمل</th>
              ...
              {isLoading ? (
                ...
                filteredWomen?.map((woman) => (
                  <tr key={woman.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-6 py-4">الشهر {woman.pregnancyMonth}</td>
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${
                        ...
                        woman.healthStatus === 'risk' ? 'bg-red-50 text-red-700 border-red-200' :
                        'bg-yellow-50 text-yellow-700 border-yellow-200'
                      }`}>
                        {getHealthStatusTranslation(woman.healthStatus)} {/* Apply health status translation */}
                      </span>
                    </td>
                    <td className="px-6 py-4">{getCampName(woman.campId)}</td>
                    <td className="px-6 py-4 text-left"> {/* text-left for RTL */}
                      <div className="inline-flex items-center gap-1 rounded-md border bg-card p-1 shadow-sm">
                        <Link href={`/pregnant-women/${woman.id}`}>
                          <Button variant="ghost" size="icon" className="h-8 w-8">
                            <Eye className="h-4 w-4 text-blue-500" />
                          </Button>
                        </Link>
                        <Link href={`/pregnant-women/${woman.id}/edit`}>
                          <Button variant="ghost" size="icon" className="h-8 w-8">
                        ...
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setWomanToDelete(woman.id)}>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                ...
                                المرأة الحامل بشكل دائم وإزالة بياناتها من خوادمنا.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                              <AlertDialogCancel onClick={() => setWomanToDelete(null)}>إلغاء</AlertDialogCancel>
                              <AlertDialogAction onClick={() => {
                                if (womanToDelete !== null) {
                                  deleteWoman(womanToDelete);
